...
function CourseDetails() {
  ...
  <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left'>
    ...
    {/* Background gradient */}
    <div className='absolute top-0 left-0 w-full h-screen -z-1 bg-gradient-to-b from-cyan-100/70'></div>
    ...
    (calculateRating(courseData)) ? assets.star : assets.star_blank
              } className='w-3.5 h-3.5'/>))}</div>
              <p className='text-blue-600'>({courseData.courseRatings.length} {courseData.courseRatings.length > 1 ? 'ratings' : 'rating'})</p>

              <p>{courseData.enrolledStudents.length} {courseData.enrolledStudents.length > 1 ? 'students' : 'student'}</p>
     </div>
     <p className='text-sm'>Course by <span className='text-blue-600 underline'>Nirajjj</span></p>
      ...
      <div className='pt-5'>
        {courseData.courseContent.map((chapter, index) => (
          <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
            ...
            style={{
            maxHeight: openSections[index] ? `${chapter.chapterContent.length * 100}px` : '0px',
             }}>
    ...
    <div className='max-w-course-card z-10 shadow-custom-card rounded-t md:rounded-none overflow-hidden bg-white min-w-[300px] sm:min-w-[420px]'>
      ...
      <div className='p-5'>
        <div className='flex item-center gap-2'>
          ...
          <p className='text-red-500'><span className='font-medium'>5 days left at this price!</span></p>
        </div>

        <div className='flex gap-3 items-center pt-2'>
          <p className='text-gray-800 md:text-4xl text-2xl font-semibold'>{currency}{(courseData.coursePrice - courseData.discount * courseData.coursePrice/ 100).toFixed(2)}</p>
          <p className='line-through'>{currency}{courseData.coursePrice}</p>
          <p className='text-green-500'>{currency}{courseData.discount}% off</p>
        </div>
